...
/**
 * Use this class to initialize the crash reporting feature using {@link #init(Application)} as soon
 * as possible in your {@link Application} subclass {@link Application#onCreate()} method.
 * Configuration items must have
 * been set by using {@link ReportsCrashes} above the declaration of your {@link Application}
 * subclass.
 * 
 * @author Kevin Gaudin
 */
public class ACRA {
	...
	/**
	 * <p>
	 * Initialize ACRA for a given Application. The call to this method should be placed as soon as
	 * possible in the {@link Application#onCreate()} method.
	 * </p>
	 * 
	 * @param app
				...
				if (pm.checkPermission(permission.INTERNET, mApplication.getPackageName()) == PackageManager.PERMISSION_GRANTED) {
					...
					if (mReportsCrashes.formUri() != null && !"".equals(mReportsCrashes.formUri())) {
						...
						if (mReportsCrashes.formKey() != null
								&& !"".equals(mReportsCrashes.formKey().trim())) {
							errorReporter.addReportSender(new GoogleFormSender(mReportsCrashes
									.formKey()));
						}
					}
				} else {
	...
	public static SharedPreferences getACRASharedPreferences() {
		if (!"".equals(mReportsCrashes.sharedPreferencesName())) {
			Log.d(ACRA.LOG_TAG,
					"Retrieve SharedPreferences " + mReportsCrashes.sharedPreferencesName());
	...
	/**
	 * Default list of {@link ReportField}s to be sent in email reports {@see #mailTo()}. You can
	 * set your own list with {@link #reportFields()}.
	 */
	public final static ReportField[] DEFAULT_MAIL_REPORT_FIELDS = {
			ReportField.USER_COMMENT, ReportField.ANDROID_VERSION,
			ReportField.APP_VERSION_NAME, ReportField.BRAND, ReportField.PHONE_MODEL,
	...
	public static final ReportField[] DEFAULT_REPORT_FIELDS = {
			...
			AVAILABLE_MEM_SIZE,
			CUSTOM_DATA, IS_SILENT, STACK_TRACE, INITIAL_CONFIGURATION, CRASH_CONFIGURATION,
			DISPLAY, USER_COMMENT,
			USER_EMAIL, USER_APP_START_DATE, USER_CRASH_DATE, DUMPSYS_MEMINFO, DROPBOX, LOGCAT,
			EVENTSLOG, RADIOLOG,
	...
	/**
	 * A special String value to allow the usage of a pseudo-null default value in annotation
	 * parameters.
	 */
